...
type dispatchActions = { type: 'SET_LOADING', payload: boolean }
  | { type: 'SET_ERROR', payload: boolean }
  | { type: 'SET_ERRORMESSAGE', payload: string }
  | { type: 'SET_ERRORTYPE', payload: string }
  | { type: 'SET_RESPONSE', payload: object }

function fetchReducer(state: IntialStateType, action: dispatchActions): IntialStateType {
    ...
    async function fetchReponse() {
      try {
        if (url) {
          ...
          dispatch({ type: 'SET_ERRORMESSAGE', payload: `URL is missing or wrond provided` })
        }
      } catch (error: any) {
